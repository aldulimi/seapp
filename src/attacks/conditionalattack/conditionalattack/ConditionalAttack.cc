...
bool ConditionalAttack::isMatchingPacketFilter (cPacket* packet) const {
	...
	if (packet->par("isFiltered").boolValue()) {
		EV << "[ConditionalAttack::isMatchingPacketFilter] the packet was already filtered before" << endl;
		return false;	
	}
    ...
    // check if the current packet matches the packet filter
    return (packetFilter->isMatchingPacketFilter(packet));
